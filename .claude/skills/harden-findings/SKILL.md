...
description: Sweep the code-review reports in specification/implementation/ for still-unfixed HIGH/MEDIUM findings, fix each with a regression test, update the reports in place, and (opt-in) ship the result as a ZZ patch release. Runs standalone, or invoked automatically at each phase boundary by /ship-phase and /ship-solution.
---
...
- `scope` — optional filter: a phase (`v2` → reports whose findings belong to that phase's code),
  a version (`v2.02`), or omitted → **all** `specification/implementation/*code-review*.md` reports.
- `--release` — after fixes land, cut the `ZZ` patch release automatically (e.g. `v2.03.00` →
  `v2.03.01`, tag `v2.03.01`). Without it, finish by **recommending** `/release-version` —
...
1. Confirm the working dev branch and a clean tree; establish a **green baseline** (`--import` clean + `test_worldgen.sh` green). Never harden on a red suite — fix a clear flake first (own commit) or surface it.
2. **Collect** the code-review reports: `specification/implementation/*code-review*.md`, filtered by `scope`
   if given.
...
4. **Commit** one focused change referencing the finding (`fix(<area>): … (code review #N)`), with the
   `Co-Authored-By` trailer. A **seam change** (the map language, `interact`/`hit`, the `Game` signals, collision layers, a `data/` table)
...
5. **Update the source report in place:** flip the finding's **Status** to `✅ FIXED — <commit>`,
   ...
   (e.g. v5.01 resilience), note there that this scope is **already addressed** so it isn't re-done.
...
1. Re-run the **full suite** once after the sweep — green and deterministic — plus `check_assets.py`.
...
3. **Release:** if `--release` was passed (or the user confirms when asked), invoke `release-version`
   ...
   `01.04.01`, tag `v1.04.01`). Otherwise just recommend the command and stop — releasing stays
   explicit.

### Step 2.5: Emit tracking events

`--emitter skill:harden-findings --scope phase=..,version=..`: on entry → `harden.start`; each landed
fix → `harden.finding.fixed` (`finding`, `sha`, **plus `test_worldgen.sh` with the full-suite counts after the
...
- **Stop on failure.** A red `test_worldgen.sh`/`check_assets.py` on any fix halts the sweep — report what landed and what
