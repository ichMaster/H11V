...
description: Review one version's already-generated specification/implementation/vA.B-issues.md against the REAL current implementation and correct any issue that has drifted (stale file names, changed signatures, evolved contracts, or work already done). Edits the issues file in place with a visible "Reconciled" change-mark. Corrects issues only - never implements code or changes the version.
---
...
`specification/implementation/vA.B-issues.md`, compare each issue's assumptions against the **actual current
...
- `/reconcile-issues v3.2` → reconciles `specification/implementation/v3.2-issues.md`

## Instructions

### Step 0: Read the issues + the real implementation

1. Resolve the target to `specification/implementation/vA.B-issues.md` and read it fully (summary table,
   dependency tree, each `### H11V-### …` section).
...
3. Read the prior versions' `specification/implementation/*-execution-report.md` and any
...
- Wrong or renamed **file/module** paths (e.g. `mods/h11_world/blocks.lua` vs the real `nodes.lua`).
...
2. **Add a visible change-mark** as a blockquote directly under the issue heading, e.g.:
   ...
   > the shipped `nodes.lua` reading its definitions from the `NODES` table. Reason: matches the real
   > implementation.
