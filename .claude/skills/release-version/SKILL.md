...
Examples:
- `/release-version 1.1.0` -- bump to 1.1.0, prompt for changelog
- `/release-version 1.2.0 SQLite Repository; move-log reconstruction` -- bump with provided changelog items
...
Version notation `vA.B.C`: `XX` = roadmap version (v1→01 … v5→05), `YY` = phase
within that version (`v1.2`→YY=02), `ZZ` = post-release fix on that phase. So roadmap
phase `vA.B` → release `vA.B.00`; a fix after it bumps `ZZ` (e.g. v1.2 → `1.2.0`,
a follow-up fix → `1.2.1`). Releases are cut per phase. **Never change the version
...
3. **`project.luanti`**: update `config/description` only if it carries a version; the engine's
   `config/features` line is **never** touched.
...
`README.md` or `VERSION` exist, and `git add` is **fatal** on a pathspec that matches
...
```bash
for f in VERSION README.md RELEASE.txt CLAUDE.md; do
...
- **Don't modify source files.** This skill only touches version metadata (VERSION, README.md, RELEASE.txt, CLAUDE.md), never game logic, assets or tools.
