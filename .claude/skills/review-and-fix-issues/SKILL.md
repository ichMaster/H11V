...
- `/review-and-fix-issues v1.04` — review the released v1 phase/version (through its tag).
...
1. Resolve `target`: a version/phase (`vA`/`vA.B`) or its tag; a component (`games`/`server`/
...
3. **Establish a green baseline** — run `--import` and `test_worldgen.sh`. If the suite is **red or flaky**,
...
the collision-layer model, the `Game` signal set, and the `Level` contract entities depend on.
...
- **Input hygiene & secrets:** unvalidated table data reaching the registration loop; a brain response trusted without validation; the device password reaching or being logged
...
findings against the specs; if a gap is *already scheduled* for a later phase (e.g. v5.01), note that
rather than treating it as new.

### Step 2: Write the recommendations document (the plan)

Write **one** doc at `specification/implementation/<scope>-code-review.md` (e.g. `v1.04-code-review.md`, or
...
- **DEFER →** = larger resilience work, or anything already owned by a later roadmap phase — give the
  home (`v5.01`, `v2.03`, "cleanup/`/simplify`", "documented MVP scope"). Do **not** pull these
  forward.
...
3. **Validate:** `test_worldgen.sh` green and `check_assets.py` clean. Only commit code that passes.
...
5. **Seam changes** (the map language, `interact`/`hit`, the `Game` signals, collision layers, a `data/` table) update
...
- Add a **"Fixes applied"** section: per fix, the change, the regression test, and the verification
  (final `test_worldgen.sh` and `check_assets.py` status). For any fix that **changed a documented contract or a
...
`finding.fixed` (`sha`, **plus `test_worldgen.sh` with the full-suite counts after the fix**) for each fix-now
item; deferred ones → `finding.deferred` (`home`).

**Carry the suite size on every `finding.fixed`** — `--data '{"finding":"…","sha":"…","smoke":
...
homes); the final green `test_worldgen.sh` and `check_assets.py` status. If fixes landed on an already-released phase,
...
- **Green before, green after.** Establish a green baseline; only commit code that passes `test_worldgen.sh` +
  `check_assets.py`; keep the gates deterministic.
