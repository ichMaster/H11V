...
> **Terminology (per [game_specification.md](../../../specification/VISION.md) §6):** a **phase**
> is a top-level roadmap block `vA` (Phase 1 → `v1` … Phase 5 → `v5`); a **version** is a `vA.B`
> sub-version inside it, released as `vA.B.00`. ⚠️ The roadmap's header and the four sub-skills use
...
for each PHASE vA in PLAN (in roadmap order):
    for each VERSION vA.B of that phase in PLAN (in order):
        ...
        5. release-version vA.B.00           ← RELEASE PER VERSION (tag vA.B.00)
...
A **selector** is a phase (`vA`), a version (`vA.B`), or a range (`vA-vYY`). Pass **one or a
comma-separated list of any mix** — the whole list is expanded into a single ordered plan.

- `/ship-phase v2` — ship **phase v2**: every version in it (v2.1 → v2.02 → v2.03), each through
...
- `/ship-phase v2.02` — ship the single **version v2.02** (steps 0–5, incl. its release), then
...
- `/ship-phase v1.1,v1.03,v2` — a **mixed list**: two individual versions plus a whole phase.
- `/ship-phase v1-v2,v4.01 --no-harden` — a range plus a single version, with no hardening sweeps.
...
> in the v2 you left out. Anything already released is then skipped, so on a repo built up to v5.02,
> `/ship-phase v5.3` still does exactly one version's work. You name the destination; the skill works
...
1. **Parse the selector list.** Split the argument on commas and trim whitespace. Each element is a
   **phase** (`vA`), a **version** (`vA.B`), or a **range** (`vA-vYY`); a single element is just a
...
3. **Close the set under its dependencies — add every version the plan needs but the user didn't
   name.** The roadmap is strictly cumulative: v3 (Web UI) cannot be built without v1's engine and
   server or v2's agent. So take the **highest** version in the set and add **every roadmap version
   that precedes it** and isn't already there. Missing prerequisites are executed, not warned about.
   - `/ship-phase v3` → plans v1.1 … v3.03, not just v3's three versions.
   - `/ship-phase v1,v3` → the omitted v2.1–v2.03 are filled in.
   - `/ship-phase v5.3` → plans the whole roadmap up to and including v5.3.

   This is safe precisely because of step 7: any filled-in version that is **already shipped** (its
   release tag exists) is skipped, so on a repo that is built up to v5.02, `/ship-phase v5.3` still
...
5. **Reject nothing silently.** If a selector doesn't resolve to a real roadmap phase/version — a typo,
   an out-of-range `v09`, a reversed range (`v3-v1`) — name the offending element and ask. Never drop
...
6. Confirm we are on the working dev branch and the tree is clean; establish a **green baseline**
   (`--import` clean + `test_worldgen.sh` green). Never start on a red suite — fix a clear flake first or surface it.
7. **Skip already-shipped versions** (release tag `vA.B.00` exists). This is what keeps step 3's
...
```
selectors : v3.2 · v1
expanded  : v3.2 | v1.1 v1.2 v1.03 v1.04
de-duped  : (no overlap)
filled    : + v2.1 v2.02 v2.03 v3.01     ← prerequisites of v3.2, not named by the user
ordered   : v1.1 → v1.2 → v1.03 → v1.04 → v2.1 → v2.02 → v2.03 → v3.01 → v3.2
shipped?  : v1.1–v1.04 tagged already → skipped

PLAN (5 versions to run)
  v2  v2.1, v2.02, v2.03    → HARDEN → report
  v3  v3.01, v3.2            → HARDEN → report

ℹ filled in v2.1–v2.03 and v3.01: v3.2 cannot build without them.
ℹ reordered: v3.2 was listed first, ships last — roadmap order is required.
ℹ skipped v1.1–v1.04: already released.
```

Note what the fill did **not** cost: v1 was named by the user but is already shipped, so it drops out;
v2 and v3.01 were never named but are genuinely missing, so they run. The plan is the *work actually
...
| **Size mix** | From the Tasks + DoD: work touching a seam (the map language, `interact`/`hit`, the `Game` signals, collision layers, a `data/` table shape) skews **M/L**; additive work inside an existing script skews **S/M**. Convert to points (S=1, M=3, L=5) — the burn-down is size-weighted, so counts alone are not enough. |
...
5. **`release-version vA.B.00`** → bump `VERSION`/`RELEASE.txt`/the app version, tag `vA.B.00`
...
- **`--no-harden` was passed** → skip it entirely. The deferred HIGH/MEDIUM findings stay in their
  documented homes (e.g. v5.01) and are listed as still-outstanding in the phase report.
...
latest version (e.g. `v2.03.01`). Its escape hatch (a fix that can't land safely is held with a
...
- **Release per VERSION (`vA.B.00`)** — after that version is built, reviewed, and its fix-now items
...
- **Stop on failure — do not paper over it.** If any sub-skill fails, or any fix hits a red
  `test_worldgen.sh`/`check_assets.py`, halt, report what completed and what remains, and let the user decide. Never release
...
- **Delegate, never duplicate.** This skill only sequences the sub-skills (`generate-issues`,
  ...
  `H11V-###` namespace, releases use unprefixed `vA.B.C` tags, every line generated fresh.
