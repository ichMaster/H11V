...
- `/ship-solution` — ship **the whole solution**: every version with a `specification/implementation/
...
- `/ship-solution v1.1,v1.03,v2` — a **mixed list**: two individual versions plus a whole phase.
...
> then skipped, so on a repo built up to v5.02, `/ship-solution v5.3` does exactly one version's
...
1. **Parse the selector list.** Split the argument on commas and trim whitespace; each element is a
   **phase** (`vA`), a **version** (`vA.B`), or a **range** (`vA-vYY`). **No argument → all**
   ...
   `v09`, a reversed range (`v3-v1`) — **name it and ask**; never drop it and ship the rest.
...
3. **Close the set under its dependencies.** The roadmap is cumulative — v3 (Web UI) cannot be built
   without v1's engine and server or v2's agent. Take the **highest** version in the set and add
...
4. **Resolve each version in the closed set to one of three states**, in this order:
   - **Already released** (tag `vA.B.00` exists) → **skip**; the dependency is satisfied. This is what
     keeps the fill cheap: on a repo built to v5.02, `/ship-solution v5.3` still runs one version.
...
6. Confirm the working dev branch + a clean tree; establish a **green baseline** (`--import` clean + `test_worldgen.sh` green) and **record the baseline test count** (the "before" for statistics). Never start red.
...
resolved  : v1.1–v1.04 released → skipped
            v2.1–v2.03, v3.01, v3.2 → have issues files → included
ordered   : v2.1 → v2.02 → v2.03 → v3.01 → v3.2

PLAN (5 versions to run)
  v2  v2.1, v2.02, v2.03    → HARDEN → phase row
  v3  v3.01, v3.2            → HARDEN → phase row
...
Had `v2.02` lacked an issues file, the run would **stop at Step 0.4** rather than skipping it — v3.2
...
2. **`execute-issues-file vA.B`** — implement each issue from the reconciled file in dependency order
   → validate (`--import` clean + `test_worldgen.sh` green, **LLM mocked**) → commit (one issue = one commit) → push;
...
4. **`release-version vA.B.00`** — bump + tag `vA.B.00` + push.
...
- Suite: <baseline> → <final> tests passing · check_assets.py clean · zero paid calls
...
- **Sequential and gated; stop on failure.** Any sub-skill failure or a red `test_worldgen.sh`/`check_assets.py` halts the
